...
def make_str_from_column(board, column_index):
    ...
    s = ''
    for letterlist in board:
        s += letterlist[column_index]
...
def board_contains_word_in_column(board, word):
    ...
    for colunm_index in range(lb):
        if word == make_str_from_column(board, colunm_index):
            return True
...
def update_score(player_info, word):
    ...
    if is_valid_word(player_info, word):
        score = player_info[1] + 1 
